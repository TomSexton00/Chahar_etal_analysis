...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <input cbinned> <input cbins> <input mat> <max distance> <output file>\n";
	exit 1;
}
...
open(IN, $cbinned) || die;
print STDERR "Reading file $cbinned into hash...\n";
$header = <IN>;
...
foreach my $bin (sort sort_bins keys %bin_counters)
{
	...
	my $ucount = $bin_counters{$bin}->{count};
